...
To use `EQ_STORAGE_BACKEND` as `datastore` or `EQ_SUBMISSION_BACKEND` as `gcp` directly on GCP and not a docker image, you need to set the GCP project using the following command:
```
